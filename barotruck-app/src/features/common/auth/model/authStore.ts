...
export const useAuthStore = create<AuthState>((set, get) => {
  const persistUsers = async (nextUsers: StoredUser[]) => {
    await saveUsers(nextUsers);
    set({ users: nextUsers });
  };

  const getUsersSafe = async () => {
  ...
  return {
    ...
    resetPasswordWithCode: async ({ email, code, newPassword }) => {
      ...
      nextUsers[idx] = { ...nextUsers[idx], password: pw };
    ...
    resetPassword: async ({ email, code, newPassword }) => {
      return get().resetPasswordWithCode({ email, code, newPassword });
    },
  };
});
