...
function mapTon(v: string | null) {
  ...
  switch (v) {
    case "1T": return "1t";
    case "1_4T": return "1.4t";
    case "2_5T": return "2.5t";
    case "3_5T": return "3.5t";
    case "5T": return "5t";
    case "11T": return "11t";
    case "25T": return "25t";
    default: return "1t";
...
function SelectField({
  ...
  const s = useMemo(() => StyleSheet.create({
    wrap: { minHeight: 56, borderRadius: 16, paddingHorizontal: 16, backgroundColor: c.bg.surface, borderWidth: 1, borderColor: c.border.default, flexDirection: "row", alignItems: "center", justifyContent: "space-between" },
    text: { fontSize: 16, fontWeight: "800", color: c.text.primary },
    placeholder: { color: c.text.secondary },
    sheetBackdrop: { flex: 1, backgroundColor: withAlpha("#000000", 0.35), alignItems: "center", justifyContent: "flex-end" },
    sheet: { width: "100%", backgroundColor: c.bg.surface, borderTopLeftRadius: 22, borderTopRightRadius: 22, paddingTop: 10, paddingBottom: 16, borderTopWidth: 1, borderTopColor: withAlpha(c.border.default, 0.8) },
    sheetTitleRow: { paddingHorizontal: 18, paddingVertical: 12, flexDirection: "row", alignItems: "center", justifyContent: "space-between" },
    sheetTitle: { fontSize: 16, fontWeight: "900", color: c.text.primary },
    option: { paddingHorizontal: 18, paddingVertical: 14, flexDirection: "row", alignItems: "center", justifyContent: "space-between" },
    optionText: { fontSize: 16, fontWeight: "800", color: c.text.primary },
    divider: { height: 1, backgroundColor: withAlpha(c.border.default, 0.6) },
  }), [c]);
...
export default function SignupDriverScreen() {
  ...
  const { email = "", password = "", phone = "" } = useLocalSearchParams<{ email: string; password: string; phone: string }>();
  ...
  const onCheckNickname = useCallback(async () => {
    if (nickname.trim().length < 2) return showMsg("알림", "2자 이상 입력하세요.");
    try {
  ...
  const s = useMemo(() => {
    ...
    return StyleSheet.create({
      ...
      header: { paddingHorizontal: S.lg, paddingTop: S.md, paddingBottom: S.md },
      backBtn: { width: 40, height: 40, borderRadius: 20, alignItems: "center", justifyContent: "center" },
      titleWrap: { paddingHorizontal: S.lg, paddingTop: 12 },
      title: { fontSize: 30, fontWeight: "900", color: c.text.primary, lineHeight: 38 },
      subtitle: { marginTop: 10, fontSize: 16, fontWeight: "700", color: c.text.secondary, lineHeight: 22 },
      form: { paddingHorizontal: S.lg, paddingTop: S.xl, paddingBottom: 140 },
      label: { fontSize: 14, fontWeight: "900", color: c.text.secondary, marginBottom: 8 },
      row: { flexDirection: "row", alignItems: "center" },
      rowGap: { width: 12 },
      tfWrap: { minHeight: 56, borderRadius: 16, paddingHorizontal: 16, backgroundColor: c.bg.surface, borderWidth: 1, borderColor: c.border.default },
      tfInput: { fontSize: 16, fontWeight: "800", paddingVertical: 0 },
      miniBtn: { height: 56, paddingHorizontal: 14, borderRadius: 16, borderWidth: 1, borderColor: c.border.default, backgroundColor: c.bg.muted, alignItems: "center", justifyContent: "center" },
      miniBtnText: { fontSize: 15, fontWeight: "900", color: c.text.primary },
      helper: { marginTop: 8, fontSize: 13, fontWeight: "800", color: c.text.secondary },
      grid2: { flexDirection: "row", alignItems: "center" },
      col: { flex: 1 },
      bottomBar: { position: "absolute", left: 0, right: 0, bottom: 0, paddingHorizontal: S.lg, paddingBottom: S.lg, paddingTop: 12, backgroundColor: withAlpha(c.bg.surface, 0.98), borderTopWidth: 1, borderTopColor: withAlpha(c.border.default, 0.7) },
      submitBtn: { height: 76, borderRadius: 20, shadowColor: withAlpha(c.brand.primary, 0.35), shadowOpacity: 1, shadowRadius: 18, shadowOffset: { width: 0, height: 10 }, elevation: 6 },
    });
  }, [c]);
